...
2. [Create a deidentified version of datasets](#deidentify)    
    1. medstar_demo    
    2. medstar_detect_demo   
    3. merged     
        - Create unique person identifier     

3. [Save processed/deidentified data](#save)      
...
First, create a unique person identifier.

Assuming first name, last name, and date of birth uniquely identifies people in this data.
...
```{r}
merged_screening_recoded_na <- merged_screening_recoded_na %>% select(-person)
```
...
```{r}
sum(is.na(merged_screening_recoded_na_deidentified$person_id)) # 0
```
