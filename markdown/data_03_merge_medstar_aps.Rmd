...
medstar_detect_w_pair %>% 
  filter(
    incident_call_number %in% (
      response_ids %>% 
        semi_join(medstar_detect_w_pair, by = c("response_num" = "incident_call_number")) %>% 
        pull(response_num)
    )
  ) %>% 
...
medstar_detect_w_pair <- medstar_detect_w_pair %>% 
  mutate(
    compliance_match = incident_call_number %in% pull(response_ids, response_num) %>% as.integer()
  )
