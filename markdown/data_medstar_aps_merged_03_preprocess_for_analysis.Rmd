...
```{r}
about_data(detect_5wk) # 66,200 observations and 50 variables
```
...
```{r}
about_data(detect_5wk) # 66,200 observations and 51 variables
```
...
detect_5wk <- detect_5wk %>% 
  map_at(
    .x  = .,
    .at = select(detect_5wk, unusual_odor36:adls61) %>% names(),
...
```{r}
feather::write_feather(detect_5wk, "../data/detect_5wk.feather")
```
