...
```{r}
about_data(medstar_aps_merged) # 101,450 observations and 50 variables
```
...
```{r}
about_data(medstar_aps_merged) # 101,450 observations and 51 variables
```
...
medstar_aps_merged %>% 
  summarise(
    `Total Rows` = n(),
    `Study IDs` = id %>% unique() %>% is.na() %>% `!` %>% sum(),
    `Unique Incidents` = incident_call_number %>% unique() %>% is.na() %>% `!` %>% sum(),
    `Unique PCR` = incident_pcr_number %>% unique() %>% is.na() %>% `!` %>% sum(),
    `Unique Case Numbers` = case_num %>% unique() %>% is.na() %>% `!` %>% sum(),
    `Unique People` = person_id %>% unique() %>% is.na() %>% `!` %>% sum()
  ) %>% 
