...
```{r}
about_data(detect_5wk) # 98,968 observations and 83 variables
```
...
```{r}
about_data(detect_1id) # 2,611 observations and 83 variables
```
...
n_screen_pos_match_aps <- nrow(screen_pos_match_aps) %>% format(big.mark = ",") # 20
```

```{r}
screen_neg_match_aps   <- screen_neg %>% filter(!is.na(case_num))
n_screen_neg_match_aps <- nrow(screen_neg_match_aps) %>% format(big.mark = ",") # 22
```

```{r}
screen_pos_no_match_aps   <- screen_pos %>% filter(is.na(case_num))
n_screen_pos_no_match_aps <- nrow(screen_pos_no_match_aps) %>% format(big.mark = ",") # 189
```

```{r}
screen_neg_no_match_aps   <- screen_neg %>% filter(is.na(case_num))
n_screen_neg_no_match_aps <- nrow(screen_neg_no_match_aps) %>% format(big.mark = ",") # 1,014
```
