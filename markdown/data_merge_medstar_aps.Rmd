...
4. [Check response numbers reported to MedStar compliance](#check-response)     
...
```{r}
client_data <- feather::read_feather("/Users/bradcannell/Desktop/client_data.feather")
about_data(client_data) # 747 observations and 13 variables
```

```{r}
medstar_detect <- feather::read_feather("/Users/bradcannell/Desktop/medstar_detect.feather")
...
```{r}
allegations <- feather::read_feather("/Users/bradcannell/Desktop/allegations.feather")
about_data(allegations) # 1,051 observations and 4 variables
```

```{r}
closure <- feather::read_feather("/Users/bradcannell/Desktop/closure.feather")
about_data(closure) # 747 observations and 3 variables
```

```{r}
disposition <- feather::read_feather("/Users/bradcannell/Desktop/disposition.feather")
...
about_data(client_data_compare) # 747 observations and 5 variables
```

```{r}
medstar_compare <- medstar_detect %>%
  mutate(
    birth_mnth = month(dob),
    birth_day  = day(dob),
    birth_year = year(dob)) %>%
...
about_data(medstar_detect_w_pair) # 132 observations and 45 variables
```
...
# Check response numbers reported to MedStar compliance {#check-response}
...
We begin with 14 response numbers saved from the MedStar legal compliance data.
...
```{r eval=FALSE}
response_ids %>% 
  anti_join(medstar_detect_w_pair, by = "response_num") %>%  # 7 response numbers
  unique()
```


There are 7 response numbers in the MedStar compliance data that don't appear in the MedStar DETECT screening items data that was matched to APS cases. I have an email from MedStar from 2016-10-10 outlining the discrepancy. One response id had all nulls for the DETECT screening items. One response id could not be found in the query. The rest were ineligible for the DETECT screening tool because of age.
...
medstar_detect_w_pair %>% 
  filter(
    response_num %in% (
      response_ids %>% 
        semi_join(medstar_detect_w_pair, by = "response_num") %>% 
        pull(response_num)
    )
  ) %>% 
  arrange(response_num) %>% 
  summarise(
    `Unique Numbers` = unique(response_num) %>% length(), # 7
    `Total Rows`     = n() # 11
  )
```
...
medstar_detect_w_pair <- medstar_detect_w_pair %>% 
  mutate(
    compliance_match = response_num %in% pull(response_ids, response_num) %>% as.integer()
  )
...
about_data(append_aps_detect_w_pair) # 264 observations and 50 variables
```
...
about_data(append_aps_detect_w_pair) # 182 observations and 50 variables
```
...
about_data(append_aps_detect_w_pair) # 112 observations and 50 variables
```
...
about_data(append_aps_detect) # 56 observations and 52 variables
```
...
* **Response number definition**: From MedStar: There is incident and response number. Incident is one incident. Response is unique to patient/truck.
...
```{r}
append_aps_detect %>% 
  mutate(compare = paste0(first_name, last_name, response_date)) %>% 
  select(compare, everything()) %>% 
  arrange(compare) %>% 
  group_by(compare) %>% 
  mutate(count = row_number() %>% max()) %>% 
  filter(count > 1) %>% # Stop here to view duplicates
  summarise(Duplicates = duplicated(compare) %>% sum()) %>% 
  ungroup() %>% 
  summarise(
    `Duplicate Combinations` = n(), # 9
    `Duplicate Rows`         = sum(Duplicates) # 10
  ) 
```


There are 9 combinations of first/last name and response date that appear in more than one row. One combination appears in three rows (2 duplicates) for a total of 10 rows with a duplicated combination (19 rows total) of in terms of first/last name and response date. Investigate these manually.
...
about_data(append_aps_detect) # 46 observations and 52 variables
```
...
```{r}
append_aps_detect %>% 
  summarise(`Unique Cases` = unique(case_num) %>% length())
```
