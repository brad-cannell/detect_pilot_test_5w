...
1. [Import APS client info data](#client-info)      
2. [Import APS allegations data](#allegations)    
3. [Import APS Closure Reason](#closure)      
4. [Import APS allegation disposition data](#disposition)     
5. [Save data for merge with screening data](#save)     
...
max(client_data$intake_start) # 2015-09-17 to 2015-11-05
```
...
There are **747 observations and 13 variables** in the APS client info data.
...
There are **1,051 observations and 4 variables** in the APS allegations data.
...
There are **747 observations and 3 variables** in the APS closure reason data
...
# Import APS [allegation disposition](http://www.dfps.state.tx.us/handbooks/APS/Files/APS_pg_2700.asp#APS_2700) data {#disposition}   
...
```{r}
disposition %>% 
  group_by(case_num, allegation, perp) %>% 
  filter(n() > 1) %>% 
  count() %>% 
  ungroup() %>% 
  select(n) # 53
...
```{r}
disposition %>% 
  group_by(case_num, allegation, perp, perp_id) %>% 
  filter(n() > 1) %>% 
  count() %>% 
  ungroup() %>% 
  select(n) # 6
...
After manually checking the 12 duplicate rows, they appear to truly be complete duplicates. I will keep one row from each set of duplicates.
...
There are **1,128 observations and 5 variables** in the APS disposition data.
...
# Save data for merge with screening data {#save}

-------------------------------------------------------------------------------

```{r}
feather::write_feather(client_data, "/Users/bradcannell/Desktop/client_data.feather")
feather::write_feather(allegations, "/Users/bradcannell/Desktop/allegations.feather")
feather::write_feather(closure, "/Users/bradcannell/Desktop/closure.feather")
feather::write_feather(disposition, "/Users/bradcannell/Desktop/disposition.feather")
```
